-- ElHub - Five Night: Hunted (Optimized)
...
local showESPText = false -- Default OFF for performance
local trackedModels = {} 
...
local function initializeHeartbeatReferences()
	...
	-- Safe loop for controller fetching
	local attempts = 0
...
local function onHeartbeatRenderStep()
	...
	-- Logic: Find closest note within perfect window
...
local function createOrUpdateESP(model, label, color, subText)
	...
	esp.Tag.Enabled = showESPText -- Toggle visibility
...
Rayfield:Notify({Title = "ElHub", Content = "Loaded.", Duration = 3})-- ElHub - Five Night: Hunted (Optimized)
local Rayfield = loadstring(game:HttpGet("https://sirius.menu/rayfield"))()
